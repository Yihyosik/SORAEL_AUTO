...
app.post("/build", async (req, res) => {
  try {
    ...
    const user = `요청: ${prompt}\n제약: 최소 단계로, 게시/발행은 dryRun=true 기준. 출력은 JSON만. 구조: {workflow_spec:{...}, make_blueprint:{...}}`;
...
app.post("/telegram/webhook", async (req, res) => {
  try {
    ...
    const b = await axios.post("http://localhost:" + PORT + "/build", { prompt: text, dryRun: true });
    const runId = b.data.runId;
    const d = await axios.post("http://localhost:" + PORT + "/deploy", { runId, dryRun: true });

    await sendTG(chatId, `✅ 완료\nrunId: ${runId}\nstatus: ${d.data.status}\nscenario: ${d.data.scenarioId}`);
    return res.json({ ok: true });
  } catch (e) {
