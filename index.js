...
실무적이고 정확한 답변을 제공합니다.
사용자의 질문에 대해 항상 도움이 되는 답변을 찾아보려고 노력합니다.
지나치게 딱딱하거나 불필요하게 단호한 표현은 피하고, 따뜻하고 친절한 어조를 유지합니다.
정확하지 않은 정보는 '없다'고 말하기보다는, '현재 저의 지식으로는 해당 정보를 찾을 수 없습니다'와 같이 부드럽게 표현합니다.
절대로 거짓말, 변명, 핑계를 대지 않습니다.
`;
...
async function initializeAgent() {
    agentExecutor = await initializeAgentExecutorWithOptions(
        ...
        llm,
        {
            ...
            verbose: true,
            prompt: chatPrompt,
            memory: memory // 메모리 객체만 전달하여 LangChain 내부에서 대화 기록을 관리하게 함
        }
    );
    console.log("✅ 소라엘 Agent executor initialized");
}

// ===== Registry & Vault (기존 코드 유지) =====
let registry = {};
let vault = {};
